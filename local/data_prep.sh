...
for reader_dir in $(find -L $src -mindepth 1 -maxdepth 1 -type d | sort); do
  echo "reader dir: $reader_dir" #  downloads/LibriSpeech/noisy-dev/3
  reader=$(basename $reader_dir) # 3
  echo "reader: $reader"
  ...
  echo "reader_gender: $reader_gender" # f
  ...
  for chapter_dir in $(find -L $reader_dir/ -mindepth 1 -maxdepth 1 -type d | sort); do
    echo "chapter directory:  $chapter_dir" # downloads/LibriSpeech/noisy-dev/3/31
    chapter=$(basename $chapter_dir)
    echo "chapter: $chapter" # 31
    ...
    find -L $chapter_dir/ -iname "*.wav" | sort | xargs -I% basename % .wav| \
      # awk -v set variable dir = downloads/LibriSpeech/noisy-dev/3/31
...
! [ "$ntrans" -eq "$nutt2spk" ] && \
  ...
  echo "Inconsistent #transcripts($ntrans) and #utt2spk($nutt2spk)"
